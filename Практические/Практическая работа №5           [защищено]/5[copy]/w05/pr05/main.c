...
// vars:
	...
	int flag;
	unsigned char last_symb;
	unsigned char curr_symb;
void main() {
						/* readme:		
						/		practice #5
...
// code:					
	...
	while (1) {
		...
		curr_symb = read();
											// 0x99 -> ' '
		if (curr_symb != last_symb && curr_symb != 0x99) {
			last_symb = curr_symb;
			disp_write(last_symb);
		}
		else if (curr_symb == 0x99)
			last_symb = ' ';
	}		
}
...
char read() {	
	...
	if (P24 == 0)
		col = 0;
	if (!(P20 == 0 || P21 == 0 || P22 == 0 || P23 == 0 || P24 == 0))
		return res;
